...
def generate_sample_data(n: int = 5000, seed: int = 42) -> pd.DataFrame:
    ...
    for i in range(1, n + 1):
        ...
        rows.append({
            ...
            # Performance & KPI
            "TargetAchievement":     round(np.clip(np.random.normal(95, 22), 30, 160), 1),
            "KPIScore":              round(np.clip(np.random.normal(72, 18), 10, 130), 1),
            "KPISubmitted":          "Y" if np.random.random() > 0.18 else "N",
            "IncentiveAmount":       int(round(max(0, np.random.normal(salary*0.07, salary*0.04)), -2)),
        })
