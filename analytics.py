...
# ── Design system ──────────────────────────────────────────────────────────
COLORS = ["#2563EB","#7C3AED","#DB2777","#16A34A","#EA580C","#0891B2",
          "#CA8A04","#DC2626","#0D9488","#9333EA","#64748B","#B45309"]
C = {"blue":"#2563EB","purple":"#7C3AED","pink":"#DB2777","green":"#16A34A",
     "orange":"#EA580C","teal":"#0891B2","red":"#DC2626","amber":"#CA8A04"}

def _L(title="", h=420):
    return dict(
        title=dict(text=f"<b>{title}</b>",
                   font=dict(size=14,color="#0f172a",family="Inter"),x=0,pad=dict(l=4)),
        font=dict(family="Inter,system-ui,sans-serif",size=11,color="#334155"),
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#f8fafc",
        height=h, margin=dict(l=10,r=10,t=50,b=10,pad=2),
        colorway=COLORS,
        hoverlabel=dict(bgcolor="white",bordercolor="#e2e8f0",
                        font_family="Inter,sans-serif",namelength=-1),
        legend=dict(bgcolor="rgba(255,255,255,0.9)",bordercolor="#e2e8f0",
                    borderwidth=1,font_size=11,orientation="h",
                    yanchor="bottom",y=1.02,xanchor="right",x=1),
        xaxis=dict(showgrid=True,gridcolor="#e2e8f0",linecolor="#cbd5e1",tickfont_size=11),
        yaxis=dict(showgrid=True,gridcolor="#e2e8f0",linecolor="rgba(0,0,0,0)",
    ...
    if prob_days and prob_days in df.columns:
        ...
        if group_col and group_col in df.columns:
            ...
            fig.update_layout(**_L(f"Time to Confirmation (Days) by {group_col}"),
                              showlegend=False,yaxis_title="Days in Probation")
        else:
...
def age_distribution(df, col_map, group_col):
    ...
    if group_col and group_col in df.columns:
        ...
        fig.update_layout(**_L(f"Age Distribution by {group_col}"),
                          showlegend=False,yaxis_title="Age",xaxis_title="")
    else:
...
def tenure_distribution(df, col_map, group_col):
    ...
    if group_col and group_col in df.columns:
        ...
        fig.update_layout(**_L(f"Tenure Distribution by {group_col}"),
                          showlegend=False,yaxis_title="Years of Service",xaxis_title="")
    else:
