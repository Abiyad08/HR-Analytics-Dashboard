...
def headcount_sunburst(df, col_map, hierarchy):
    ...
    fig = px.sunburst(data, path=path, color=path[0], color_discrete_sequence=COLORS)
    fig.update_traces(
        textinfo="label+percent parent",
        insidetextfont=dict(size=12),
        hovertemplate="<b>%{label}</b><br>%{value} employees<br>%{percentParent:.0%} of parent<extra></extra>",
    )
    fig.update_layout(**_L("Workforce Sunburst — Full Hierarchy", 520))
...
def salary_vs_tenure(df, col_map, group_col):
    ...
    fig.update_layout(**_L("Salary vs. Tenure"), yaxis_tickprefix="$", yaxis_tickformat=",")
...
def salary_by_group(df, col_map, group_col):
    ...
    return fig, f"Company median salary: ${overall:,.0f}. Green = above median, red = below."
