...
# Render KPI cards using st.columns (most reliable for Streamlit Cloud)
n_cards  = min(len(cards), 6)
cols_kpi = st.columns(n_cards, gap="small")
for i, card_html in enumerate(cards[:n_cards]):
    with cols_kpi[i]:
        # Wrap in a column-aware div so each card fills its column
        st.markdown(f"<div style='height:100%'>{card_html}</div>",
                    unsafe_allow_html=True)
...
with tabs[-1]:
    ...
    if not att_check or att_check not in df_filtered.columns:
        ...
        if "pred_result" in st.session_state:
            ...
            pred_kpi_cols = st.columns(4, gap="small")
            for i,ch in enumerate(pred_cards):
                with pred_kpi_cols[i]: st.markdown(ch, unsafe_allow_html=True)
