...
def generate(df: pd.DataFrame, col_map: dict) -> list[tuple]:
    ...
    if att_col and att_col in df.columns:
        ...
        insights.append((
            ...
            f"At a 50%-of-salary replacement cost, this represents an estimated "
            f"${df[att_col].sum() * (df[sal_col].mean() if sal_col and sal_col in df.columns else 60000) * 0.5 / 1e6:.1f}M in annual turnover cost."
            if sal_col and sal_col in df.columns else
            f"{diff:+.1f}pp vs the {ATTRITION_BENCHMARK}% industry benchmark."
        ))
    ...
    if sal_col and sal_col in df.columns:
        ...
        sal_range = df[sal_col].quantile(0.9) - df[sal_col].quantile(0.1)
        insights.append((
            "💰",
            f"Median salary is ${med_sal:,.0f}",
            f"Middle 80% salary range: ${df[sal_col].quantile(0.1):,.0f}–${df[sal_col].quantile(0.9):,.0f} "
            f"(spread of ${sal_range:,.0f}). High spread may indicate pay equity concerns."
        ))
